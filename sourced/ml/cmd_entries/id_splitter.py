...
from sourced.ml.algorithms.id_splitter import build_rnn_from_args, build_cnn_from_args
from sourced.ml.algorithms.id_splitter import train_id_splitter
...
DEFAULT_PADDING = "post"  # add padding values after the input
DEFAULT_EPOCHS = 10
...
DEFAULT_TEST_RATIO = 0.2  # fraction of the dataset used for evaluation
...
def train_id_splitter_cnn(args: argparse.ArgumentParser):
    return train_id_splitter(args, model=build_cnn_from_args)
...
def add_train_id_splitter_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("-p", "--padding", default=DEFAULT_PADDING, choices=("pre", "post"),
    ...
    rnn_parser = add_parser("rnn", "Train a RNN model to split identifiers.")
    ...
    cnn_parser = add_parser("cnn", "Train a CNN model to split identifiers.")
